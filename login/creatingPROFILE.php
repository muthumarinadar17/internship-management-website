...
$user_email = $_SESSION['email'];  // Retrieve logged-in user's email
...
if ($result->num_rows > 0) {
    ...
    $_SESSION['profileImage'] = $row['image']; // Store in session
}
// PROFILE
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['upload']) && isset($_FILES['profilePicture'])) {
        ...
        if ($conn->query($removeImageQuery) === TRUE) {
            unset($_SESSION['profileImage']); // Remove from session
        }
    }
}
...
/*--------------------------------------*/
        ...
        .form img:before {
        content: attr(alt); /* Fetch the alt text */
        color: white;
        font-size: 14px;
        font-weight: bold;
        text-align: center;
    ...
    html {
        scroll-padding-top: 390px; /* Adjust based on header size */
    }
...
// Starting position for the first section
let yPos = 30;
const sectionSpacing = 10; // Space between sections

// Add Resume Details with Proper Padding
...
// Retrieve the email from the session and store it in localStorage
