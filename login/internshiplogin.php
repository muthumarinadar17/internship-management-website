    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === 0) {
        ...
        if (move_uploaded_file($file_tmp, $upload_dir . $file_name)) {
            ...
            if (mysqli_query($conn, $insert)) {
                echo json_encode(["success" => true, "internship" => $internship]); // Return internship name
                exit();
            } else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (form) {
        form.addEventListener("submit", function(event) {
            ...
            fetch("", { // Send request to the same page
                method: "POST",
                body: formData
            })
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Function to detect Name & Email from extracted text
    function autofillDetails(text) {
    console.log("Extracted Text:", text); // Debugging purpose

    // Extract only the first two words after "Name:" (to avoid extra text like "Contact")
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    viewButtons.forEach(button => {
        button.addEventListener("click", function () {
            const product = this.closest(".product"); // Get the nearest product div
            const internshipName = product.getAttribute("data-name"); // Get the data-name value
            InternshipInput.value = internshipName; // Autofill the company name input
...
async function fetchInternships() {
    try {
        let response = await fetch("backend.php"); // Fetch internship data from backend.php
        let internships = await response.json();
        displayInternships(internships); // Call function to display internships
    } catch (error) {
        console.error("Error fetching internships:", error);
    }
}

// Function to display internships based on filtered results
function displayInternships(internships) {
    ...
    `;
        ...
        // Create hidden popup for each internship
        ...
        container.appendChild(popupDiv); // Append popups to the body
    });
}
...
// Function to filter internships based on dropdown selections
...
fetchInternships();
document.addEventListener("DOMContentLoaded", function () {
    fetchInternships(); // Fetch internships dynamically

    // Show resume upload container and autofill form on Apply button click
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("apply-btn")) {
            ...
            // Show the resume upload container
            document.querySelector(".container-apply").classList.add("active");
        }
    });

    // Close button hides the resume upload container
