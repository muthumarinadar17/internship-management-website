...
// Fetch the total number of internships
...
// Fetch the total number of submit forms 
...
<style>
    ...
    /* Apply flexbox to align labels and inputs */
    .input-group {
        display: flex;
        align-items: center;
        gap: 10px;
    ...
    <div class="content-container">
        ...
        if ($form_result->num_rows > 0) {
            while($row = $form_result->fetch_assoc()) {
                $fileName = $row["file"]; // Only filename stored, no path
                $filePath = "uploads/" . $fileName; // Assuming files are in "uploads/" folder
        ...
        document.addEventListener("DOMContentLoaded", function () {
            document.getElementById("add").addEventListener("click", async function () {
                ...
                try {
                    ...
                    let textResponse = await response.text(); // First, get the raw response
                    try {
                        let result = JSON.parse(textResponse); // Try parsing JSON
                        if (result.success) {
